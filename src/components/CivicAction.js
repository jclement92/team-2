...
class CivilAction extends Component{
    constructor(props){
        super(props);
        this.state = {
          data: [],
    ...
    updateWeek = (evt) => {
      ...
      // var data = {data:'[30]'}
      axios.post('http://127.0.0.1:5000/user/civicAction', data)
      alert('Refresh Page');
    }

    componentDidMount(){
      axios.get('http://127.0.0.1:5000/user/civicAction')
        .then(function (res) {
          // handle success
          console.log(res.data);
          this.setState({
